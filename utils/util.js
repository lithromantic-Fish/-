    ...
    wx.request({
      ...
      success: res => {
        ...
        if (typeof data.result === 'boolean' && data.result !== true) {
          ...
          handleErrApi(data.resultcode, res, options, recall);
        }
 ...
 */
function _getSessionKeyByApi(code, data, autoSaveSessionKey = true) {

  return request({
    url: config.apiUrl + '/hr/hrzp/share/w_min_login',
    data: {
      code: code,
      data: data
    },
...
function handleErrApi(result, res, options = {}, recall) {
  ...
  //SHARE_CANCEL: 301, 
  //对方已取消分享
  if (result === config.ERR_MAPPING.SHARE_CANCEL) {
    gotoPage({
      url: '/pages/resume_invalid/resume_invalid?type=1&share_code=' + options.data.share_code,
      openType: 'redirectTo'
    })
  ...
  //SHARE_CANCEL: 302,
  //分享链接已过期
  if (result === config.ERR_MAPPING.SHARE_EXPIRE) {
    // console.info('options',options)
    gotoPage({
      url: '/pages/resume_invalid/resume_invalid?type=2&share_code=' + options.data.share_code,
      openType: 'redirectTo'
    })
  ...
  //SHARE_CANCEL: 401,
  //简历不存在
  if (result === config.ERR_MAPPING.SHARE_RESUME_NONE) {
